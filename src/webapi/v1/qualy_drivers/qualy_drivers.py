...
from src.webapi.v1.qualy_drivers.req_res_qualy_driver\
    import Driver_qualification_response, Driver_qualy_create_request
...
async def get_qualys_driver(username: str):
    cmd = commands.DriverQualyGetCommand(
...
async def get_average_driver(username: str):
    cmd = commands.DriverQualyGetAverageCommand(
        driver_username=username
    )
    uow = UnitOfWorkMongo()
