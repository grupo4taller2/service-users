...
from src.webapi.v1.qualy_passenger.req_res_qualy_passenger\
    import Passenger_qualification_response, Passenger_qualy_create_request
...
async def get_qualys_passenger(username: str):
    cmd = commands.PassengerQualyGetCommand(
...
async def get_average_passenger(username: str):
    cmd = commands.PassengerQualyGetAverageCommand(
        passenger_username=username
    )
    uow = UnitOfWorkMongo()
