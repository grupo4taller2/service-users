...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column('driver_username', sa.String,
                  ...
                  primary_key=True),
        sa.Column('plate', sa.String(c.CAR_PLATE_LEN), primary_key=True),
        sa.Column('manufacturer', sa.String(c.CAR_MANUFACTURER_NAME_LEN)),
        sa.Column('model', sa.String(c.CAR_MODEL_LEN)),
        sa.Column('year_of_production', sa.Integer),
        sa.Column('color', sa.String(c.CAR_COLOR_LEN)),
